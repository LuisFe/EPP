...
def enclosable_in_circle(points,r):
    """
    Determin whether the points can be enclosed in a circle of radius
    r (km), if true, return circle, if false, return None.
    """
    ...
    for (i, p) in enumerate(shuffled):
        if c is None or not is_in_circle(c, p):
            c = _make_circle_one_point(shuffled[0 : i + 1], p)
            if not 6371*math.cos(c[1]*math.pi/180)*math.fabs(c[2]) < r:
                c = None
                break
    if c == None:
        return False, None
